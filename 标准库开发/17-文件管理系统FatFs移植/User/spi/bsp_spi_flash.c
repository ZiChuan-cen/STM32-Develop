...
void SPI_FLASH_Erase_Sector(uint32_t addr)
{
	...
	SPI_FLASH_SendByte((addr >> 0) & 0xFF); 
	FLASH_SPI_CS_HIGH();	
...
*   @arg size:要读取多少个数据
  * @retval  
  */
void SPI_FLASH_Read_Buff(uint8_t *buf,uint32_t addr,  uint32_t size)
{
	//控制片选引脚
	FLASH_SPI_CS_LOW();	
	//指令代码
...
*   @arg size:要写入多少个数据		不能超过256
  * @retval  
  */
void SPI_FLASH_Write_Buff(uint8_t *buf, uint32_t addr, uint32_t size)
{
	uint32_t count = 0;
	while(size--)
	{
